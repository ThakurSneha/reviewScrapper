...
def index():
    if request.method == 'POST':
        ...
        try:
            ...
            if searchString in db.list_collection_names(): # searching the collection with the name same as the keyword
                review = []
                for reviews in db[searchString].find({},{"_id":0}):
                    review.append(reviews)
                return render_template('results.html',reviews=list(review)) # show the results to user
            else:
